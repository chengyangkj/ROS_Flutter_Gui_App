...
| 项 | 说明 |
| --- | --- |
| 连接页 | 填写机器人（或后端所在主机）**IP**，以及后端 **HTTP 监听端口**（与 `config.yaml` 里 `web_server.port` 一致，默认 **7684**） |
...
2. 打开应用（浏览器或桌面），在连接页输入 **IP** 与 **后端 HTTP 端口**（默认 **7684**），连接成功后会拉取 **`/api/settings`** 应用 GUI 配置。
