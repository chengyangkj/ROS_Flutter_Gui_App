...
class TF2Dart {
  static final Map<String, int> _warnThrottleMs = {};

  static void _throttledWarn(String key, String message) {
    final now = DateTime.now().millisecondsSinceEpoch;
    final last = _warnThrottleMs[key] ?? 0;
    if (now - last < 5000) {
      return;
    }
    _warnThrottleMs[key] = now;
    debugPrint(message);
  }
  ...
  RobotPose lookUpForTransform(String from, String to) {
    ...
    try {
      ...
      if (path.isEmpty) {
        _throttledWarn('path|$from|$to', 'Warning: No path found from $from to $to');
      ...
      for (int i = 0; i < path.length - 1; i++) {
        ...
        if (transformList == null || transformList.isEmpty) {
          _throttledWarn('tx|$curr|$next', 'Warning: No transform found from $curr to $next');
          continue;
        }
      ...
      _throttledWarn('lookup_err', 'Error in lookUpForTransform: $e');
  ...
  List<String> shortPath(String from, String to) {
    ...
    _throttledWarn('between|$from|$to', "Warning: No path found between '$from' and '$to'");
    return [];
  }
}
